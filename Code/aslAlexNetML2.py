...
# -------------------- Model (AlexNet from the original version)
...
writer_train.add_graph(alexnet, Variable((torch.Tensor(train_loader.dataset.dataImages[0:1])).cpu(), ))
...
for epoch in range(0, epochs):
    ...
    writer_train.add_scalars("losses", {'train_an':epoch_loss, 'val_an':val_epoch_loss}, int(epoch))
    writer_train.add_scalars("accuracies", {'train_an':epoch_accuracy, 'val_an':val_epoch_accuracy}, int(epoch))

    # Learning rate scheduler update
    scheduler.step(val_epoch_loss)

writer_train.add_histogram("error_an", np.array(train_losses))
...
torch.save(alexnet.state_dict(), 'alexnetModel.pkl')
