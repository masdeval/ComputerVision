...
#cnn = torch.load('baselineModel.pkl')
#cnn.eval()
...
'''
y_true_binarized = label_binarize(y_true, classes=[i for i in range(29)])
y_pred_binarized = label_binarize(y_pred, classes=[i for i in range(29)])

classes_auc = []
for i in range(29):
    auc = metrics.roc_auc_score(y_true_binarized[:,i], y_pred_binarized[:,i])
    classes_auc.append(auc)'''

'''
fpr, tpr, _ = metrics.roc_curve(y_true, scores, pos_label=3)
metrics.auc(fpr, tpr)
'''
